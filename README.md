<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?color=014D4D&center=true&vCenter=true&size=40&width=900&height=60&lines=🌟+WELCOME+!+I'm+SNEHA+SRIVASTAVA" />
</div>

<p align="center">
  <img src="https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExb3V3NTM3enF1bWM4bGo4bjl3dzJkOHpob3JyZWh3cjJkZGg1MDJlYSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/L1R1tvI9svkIWwpVYr/giphy.gif" width="500" height="200"/>
</p>

<p align="center">
  <strong>BCA Student | Data Science & ML Enthusiast</strong><br>
  <em>Always Curious, Always Learning! 🚀</em>
</p>

---

### 🚪 Internships
- 🧠 <strong>AI Intern</strong> @Infosys Springboard *(Active)*
- Making an **IVR System**.

- 🧠 <strong>Data Science / ML Intern</strong> @ Tata Steel SNTI *(Completed)*  
  - Worked on **real-world manufacturing datasets** like CWRU bearing dataset & Steel Plate defect prediction.  
  - Applied ML models (SVM, Random Forest, Logistic Regression) for fault detection.  
  - Tools: Python, Pandas, NumPy, Scikit-learn, Matplotlib   

- 🌐 <strong>Web Development Intern</strong> @ MotionCut *(Completed)*  

- 🧩 <strong>Open-Source Contributor</strong> @ Social Summer of Code 2025  
- 🧩 <strong>Project Admin</strong> @ GirlScript Summer of Code 2025 *(FocusVerse Project)*  

---

### 📚 Projects

- 🤔 <strong>Emotion Detection</strong> – CNN model to detect emotions from facial images (TensorFlow, Keras).  
- 🔩 <strong>Steel Plate Defect Prediction</strong> – Classified surface defects using Random Forest on manufacturing dataset.  
- ⚙️ <strong>CWRU Bearing Fault Detection</strong> – Fault classification from vibration signals using ML models.  
- ⏳ <strong>FocusVerse</strong> – Pomodoro-style productivity web app (Flask, HTML, CSS, JS).  
- ✅ <strong>To-Do List (GUI)</strong> – Task management with Tkinter.  
- ✨ <strong>Password Generator</strong> – CLI-based secure password generator in Python.  
- 🌊 <strong>Rock Paper Scissors</strong> – Fun Python game using Random module.  

---

### 🔧 Skills

**Programming:** Python | Java | C++ | SQL  
**Libraries & Tools:** Pandas | NumPy | Scikit-learn | TensorFlow (basics) | Excel | Git | GitHub  
**Web Basics:** HTML | CSS | JavaScript  
**Soft Skills:** Communication | Teamwork | Problem-Solving | Time Management  

---

### 🎓 Education

- **BCA (2023 – 2026)** – Netaji Subhas University, Jamshedpur | GPA: 8.5   

---

### 📜 Certifications

- Gen AI – Job Simulation | Tata Forage *(Jun 2025)*  
- Advanced MS Excel | TCS iON *(Jun 2025)*  
- MS Office | TCS iON *(Jun 2025)*  
- Time Management | Infosys Springboard *(Jun 2025)*  

---

### 🏆 Achievements

- Selected as **Project Admin** – FocusVerse @ GSSOC 2025  
- Open-Source **Contributor** – Social Summer of Code 2025  

---

### 💬 Let's Connect!

- 📍 Jamshedpur, India  
- 📧 [Email](mailto:srivastavasneha895@gmail.com)  
- 👤 [LinkedIn](https://www.linkedin.com/in/sneha-srivastava-00273832b)  
- 🧩 [Linktree](https://linktr.ee/sneha_srivastava.2)  
- 💻 [GitHub](https://github.com/sneha842)  

---

## 📊 GitHub Stats

![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=sneha842&show_icons=true&theme=tokyonight)  
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=sneha842&layout=compact&theme=tokyonight)  
![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=sneha842&theme=tokyonight)

<p align="center">
  ✨ <i>"Dream big. Learn bigger. Grow consistently."</i> ✨
</p>
